        ...
        public async Task<Invite> GetInviteAsync(Guid token, string email, int companyId)
        {
            try
            {
                Invite invite = await _context.Invites.Where(i => i.CompanyId == companyId)
                                                      ...
                                                      .Include(i => i.Project)
                                                      .Include(i => i.Invitor)
                                                      .FirstOrDefaultAsync(i => i.InviteeEmail == email && i.CompanyToken == token);
                return invite;
        ...
        public async Task<bool> ValidateInviteCodeAsync(Guid? token)
        {
           if(token == null)
            ...
            if(invite != null)
            {
                ...
                //Custom validation of invite based on the date it was issued - invites allowed to be valid for 7 days
