    ...
    public class BTTicketService : IBTTicketService
    {
            ...
            try
            {
                if(ticket != null)
                {
                    try
                    {
                        ...
                        ticket.TicketStatusId = (await LookupTicketStatusIdAsync("Developement")).Value;
                        await _context.SaveChangesAsync();
                    }
                    catch (Exception)
                    {
        ...
        public async Task<List<Ticket>> GetAllTicketsByTypeAsync(int companyId, string typeName)
        {
            ...
            try
            {
                List<Ticket> tickets = await _context.Projects.Where(p => p.CompanyId == companyId)
                                                        ...
                                                        .Include(t => t.TicketType)
                                                        .Include(t => t.Project)
                                                        .Where(t => t.TicketStatusId == typeId)
                                                        .ToListAsync();
                return tickets;
            }
            catch (Exception)
        ...
        public async Task<List<Ticket>> GetTicketsByRoleAsync(string role, string userId, int companyId)
        {
            ...
            try
            {
                ...
                {
                    tickets = (await GetAllTicketsByCompanyAsync(companyId)).Where(t => t.OwnerUser.Id == userId).ToList();
                }
